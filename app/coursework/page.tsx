...
export default function CourseworkPage() {
  return <main>
    ...
    <section className="section shell"><div className="course-grid">{courses.map(([code,title,text]) => <article className="course-card" key={code}><span>{code}</span><h3>{title}</h3><p>{text}</p><a href="mailto:danahe.marmolejo@slu.edu?subject=Course%20information">Request course information <Arrow /></a></article>)}</div></section>
